...
import (
	...
	"github.com/agodomen/Cli-Proxy-API-Management-Center/backend/internal/auth/antigravity"
	"github.com/agodomen/Cli-Proxy-API-Management-Center/backend/internal/auth/claude"
	"github.com/agodomen/Cli-Proxy-API-Management-Center/backend/internal/auth/codex"
	"github.com/agodomen/Cli-Proxy-API-Management-Center/backend/internal/auth/kimi"
...
func matchesAuthFileLookup(auth *coreauth.Auth, name string, authIndex string) bool {
	...
	authIndexValue := lockedAuthIndex(auth)
	if name != "" && authID != name && authName != name && authIndexValue != name && filepath.Base(authID) != name {
		return false
	}
	if authIndex != "" && authIndexValue != authIndex {
		return false
	}
	return true
}
...
func (h *Handler) lookupAuthFile(name string, authIndex string) (*coreauth.Auth, bool) {
	...
	if authIndex == "" {
		...
		auths := h.authManager.List()
		for _, auth := range auths {
			if matchesAuthFileLookup(auth, name, "") {
				return auth, true
			}
		}
		return nil, false
...
func isUnsafeAuthFileName(authDir, name string) bool {
	name = strings.TrimSpace(name)
	if name == "" {
		return true
	}
	...
	absTarget, err := filepath.Abs(filepath.Join(authDir, name))
	if err != nil {
		return true
	}
	relative, err := filepath.Rel(absAuthDir, absTarget)
...
func (h *Handler) DownloadAuthFile(c *gin.Context) {
	...
	if isUnsafeAuthFileName(h.cfg.AuthDir, name) {

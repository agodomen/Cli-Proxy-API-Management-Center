// Code generated from cmd/server/main.go. DO NOT EDIT.
//
// Source: backend/cmd/server/main.go @ c9417c8ae9b16fabc0386ca35d36f13bf8b1d678
// Regenerate: bin/gen-cli-mirror.sh
//
// Package cli exposes the community CLIProxyAPI entrypoint as an importable
// package so the cpamc binary can serve both the management center and the
// full community CLI (OAuth login, TUI, Home mode, ...) from one executable,
// while cmd/server/main.go stays byte-identical to upstream.
//
// Differences from the source, all applied mechanically by the generator:
//  1. package main            -> package cli
//  2. func init()             -> func Init()
//  3. func main()             -> func Run(extraServerOptions ...api.ServerOption)
//  4. serverOptions           -> extraServerOptions prepended
//
// Version, Commit and BuildDate below remain -ldflags injection points, now
// under this package path rather than main.
package cli
...
// Init performs the process-wide logger and buildinfo setup that the community
// entrypoint does in init(). It is exported because cpamc only wants it when
// dispatching to community CLI mode.
func Init() {
...
// Run executes the community CLI entrypoint. extraServerOptions lets cpamc
// inject secondary-development server options without touching upstream code.
//
// Flags are still parsed from os.Args, exactly as upstream does.
func Run(extraServerOptions ...api.ServerOption) {

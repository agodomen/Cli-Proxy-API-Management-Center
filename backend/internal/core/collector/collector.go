...
// UsageEventSink receives successfully normalized usage events after collection.
// Probe service implements this to treat each usage report as an async probe sample.
type UsageEventSink interface {
	ProcessUsageEvents(ctx context.Context, events []usage.Event)
}
...
func (m *Manager) SetUsageEventSink(sink UsageEventSink) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.usageSink = sink
}
...
func (m *Manager) processItems(ctx context.Context, cfg RuntimeConfig, items []string) error {
	...
	return m.IngestUsageEvents(ctx, events)
}

// IngestUsageEvents persists already-normalized events from in-process runtimes.
// It shares the same SQLite and probe sink path as externally collected CPA events.
func (m *Manager) IngestUsageEvents(ctx context.Context, events []usage.Event) error {
	if len(events) == 0 {
		return nil
	}
	...
	if sink != nil && len(events) > 0 {
		// Probe processing is best-effort and must not break collection.
		sink.ProcessUsageEvents(ctx, events)
	}
	return nil
}

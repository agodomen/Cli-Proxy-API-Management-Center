...
func NormalizeRaw(raw []byte) (Event, error) {
	...
	event := Event{
		...
		LatencyMS:             latencyMS,
		Failed:                failed,
		StatusCode:            readStatusCode(record),
		ErrorMessage:          readErrorMessage(record),
...
// readStatusCode extracts HTTP status code from a record.
// It first checks top-level keys, then falls back to the nested "fail" object.
func readStatusCode(record map[string]any) int64 {
	if code := readInt(record, "status_code", "statusCode", "http_status", "httpStatus"); code != 0 {
		return code
	}
	if nested, ok := record["fail"].(map[string]any); ok {
		return readInt(nested, "status_code", "statusCode", "http_status", "httpStatus")
	}
	return 0
}

// readErrorMessage extracts the error message from a record.
// It first checks top-level keys, then falls back to the nested "fail" object
// by parsing its "body" JSON string.
func readErrorMessage(record map[string]any) string {
	if msg := readString(record, "error_message", "errorMessage", "error"); msg != "" {
		return msg
	}
	nested, ok := record["fail"].(map[string]any)
	if !ok {
		return ""
	}
	if msg := readString(nested, "error_message", "errorMessage", "error", "message"); msg != "" {
		return msg
	}
	bodyRaw := readString(nested, "body")
	if bodyRaw == "" {
		return ""
	}
	...
	// Try {"error":{"message":"..."}} format (OpenAI/Anthropic style)
	if errObj, ok := body["error"].(map[string]any); ok {
		if msg := readString(errObj, "message", "msg", "detail"); msg != "" {
			return msg
		}
	}
	return readString(body, "error_message", "errorMessage", "message", "error")
}
...
func maskSource(value string) string {
	...
	if looksSecret(trimmed) {
		...
		return "m:" + trimmed[:4] + "..." + trimmed[len(trimmed)-6:]
	}
	return trimmed
}

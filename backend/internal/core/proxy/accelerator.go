// Package proxy provides accelerator URL normalization and rewriting logic
// shared across all backend business modules (plugin-store, model-prices,
// future consumers).
//
// The canonical accelerator logic lives here in core/proxy so that community
// code (internal/pluginstore) can re-export it without owning the definitions.
// When community code is overwritten by upstream sync, the core logic remains.
package proxy

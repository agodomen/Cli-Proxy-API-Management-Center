...
type Config struct {
	...
	CORSOrigins    []string
	TLSSkipVerify  bool
	// DebugDatabases are additional SQLite files exposed by the SQL console.
	// The primary usage DB is always registered separately by the server.
	DebugDatabases []DebugDatabase
}

// LocalEngineConfig controls the CLIProxyAPI runtime embedded in the cpamc process.
...
// DebugDatabase describes a whitelisted extra SQLite database for the debug SQL console.
...
type fileConfig struct {
	...
	DebugDatabases    []DebugDatabase       `json:"debugDatabases,omitempty"`
	LocalEngine       fileLocalEngineConfig `json:"localEngine,omitempty"`
}

type fileLocalEngineConfig struct {
...
func Load() (Config, error) {
	...
	if cfgFile.LocalEngine.Enabled != nil {
		localEngineEnabled = *cfgFile.LocalEngine.Enabled
	}
	localEngineConfigPath := filepath.Join(dataDir, "cliproxyapi", "config.yaml")
	if cfgFile.LocalEngine.ConfigPath != "" {
		localEngineConfigPath = resolveConfigPath(cfgFile.LocalEngine.ConfigPath, cfgDir)
	}
	...
	return Config{
		...
		LocalEngine: LocalEngineConfig{
			Enabled:    envBool("CPAMC_LOCAL_ENGINE_ENABLED", localEngineEnabled),
			ConfigPath: env("CPAMC_LOCAL_ENGINE_CONFIG", localEngineConfigPath),
			Host:       env("CPAMC_LOCAL_ENGINE_HOST", stringFallback(cfgFile.LocalEngine.Host, "0.0.0.0")),
			Port:       envInt("CPAMC_LOCAL_ENGINE_PORT", intFallback(cfgFile.LocalEngine.Port, 18318)),
		},
		...
		DebugDatabases: loadDebugDatabases(cfgFile.DebugDatabases, cfgDir),
	}, nil
}
...
func createDefaultFileConfig(configPath string) (fileConfig, string, error) {
	...
	cfg := fileConfig{
		...
		DataDir:  "./data",
		LocalEngine: fileLocalEngineConfig{
			Enabled:    &localEngineEnabled,
			ConfigPath: "./data/cliproxyapi/config.yaml",
			Host:       "0.0.0.0",
			Port:       18318,
		},
	}
...
// loadDebugDatabases merges env USAGE_DEBUG_DB_PATHS over config.json debugDatabases.
// Env format: id:label:path,id2:path2 (label optional; path is the last colon-separated segment when 2 parts).
func loadDebugDatabases(fromFile []DebugDatabase, cfgDir string) []DebugDatabase {
	if raw := strings.TrimSpace(os.Getenv("USAGE_DEBUG_DB_PATHS")); raw != "" {
		return parseDebugDatabaseEnv(raw, cfgDir)
	}
	if len(fromFile) == 0 {
		return nil
	}
	out := make([]DebugDatabase, 0, len(fromFile))
	seen := map[string]struct{}{}
	for _, item := range fromFile {
		id := strings.TrimSpace(item.ID)
		path := strings.TrimSpace(item.Path)
		if id == "" || path == "" || id == "primary" {
			continue
		}
		if _, ok := seen[id]; ok {
			continue
		...
		out = append(out, DebugDatabase{
			ID:    id,
			Label: label,
			Path:  resolveConfigPath(path, cfgDir),
		})
	}
	return out
}

func parseDebugDatabaseEnv(raw string, cfgDir string) []DebugDatabase {
	parts := splitCSV(raw)
	out := make([]DebugDatabase, 0, len(parts))
	seen := map[string]struct{}{}
	for _, part := range parts {
		id, label, path := splitDebugDatabaseEntry(part)
		if id == "" || path == "" || id == "primary" {
			continue
		}
		if _, ok := seen[id]; ok {
...
func splitDebugDatabaseEntry(entry string) (id, label, path string) {
	entry = strings.TrimSpace(entry)
	if entry == "" {
		return "", "", ""
	}
	// Prefer splitting from the right so Windows drive letters (C:\...) stay intact
	// when using id:path or id:label:path forms with absolute unix paths.
	...
	default:
		...
		path = strings.TrimSpace(segments[len(segments)-1])
		label = strings.TrimSpace(strings.Join(segments[1:len(segments)-1], ":"))

...
describe('usageServiceApi streamUsageRealtime', () => {
  ...
  it('reconnects by throwing when the stream closes without abort', async () => {
    const encoder = new TextEncoder();
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(encoder.encode('event: ready\ndata: {"cursor":1,"at_ms":1}\n\n'));
        controller.close();
      },
    });
    ...
    await expect(
      usageServiceApi.streamUsageRealtime('https://manage.example', 'key', {
  ...
  it('throws idle timeout when no bytes arrive', async () => {
    const stream = new ReadableStream<Uint8Array>({
      start() {
    ...
    ).rejects.toThrow(/SSE stream idle timeout/);
  });
});

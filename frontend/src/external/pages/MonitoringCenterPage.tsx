...
import { DropdownMenu, type DropdownMenuItem } from '@/external/components/ui/DropdownMenu';
import {
  IconChevronDown,
  IconChevronUp,
  IconChartLine,
...
} from '@/external/features/monitoring/accountOverviewState';
import { sortAccountOverviewCardMetrics } from '@/external/features/monitoring/accountOverviewCardMetrics';
import { buildMonitoringAccountQuotaTargetsByAccount } from '@/external/features/monitoring/accountOverviewQuotaTargets';
import {
  buildMonitoringStatusBlockAriaLabel,
  getNextMonitoringStatusBlockIndex,
} from '@/external/features/monitoring/healthStatusAccessibility';
import { buildRealtimeSourceDisplayLegacy } from '@/external/features/monitoring/realtimeSourceDisplay';
import { MonitoringPanel } from '@/external/features/monitoring/components/MonitoringPanel';
import { useUsageData } from '@/external/features/monitoring/hooks/useUsageData';
import { useHeaderRefresh } from '@/hooks/useHeaderRefresh';
import { useInterval } from '@/hooks/useInterval';
import { useRequestMonitoringAvailability } from '@/external/hooks/useRequestMonitoringAvailability';
import { setStatusWithFallback } from '@/external/services/api/authFiles';
...
} from '@/external/features/monitoring/accountQuotaProviders';
import {
  formatCompactNumber,
  formatDurationMs,
  formatUsd,
...
export function MonitoringCenterPage() {
  ...
  const buildUsageQuery = useCallback(
    (nowMs: number) => {
      ...
      return {
        ...
        status: selectedStatus === 'success' || selectedStatus === 'failed' ? selectedStatus : selectedStatus === 'all' ? undefined : undefined,
  ...
  const statusOptions = useMemo(
    () => [
      ...
      // 也可以在 MonitoringCenterPage 扩展，但暂不引入数字状态码
    ],
    [t]
  );
  ...
  const handleAccountStatusToggle = useCallback(
    async (row: MonitoringAccountRow, enabled: boolean) => {
      ...
      const results = await Promise.allSettled(
        fileNames.map((fileName) => setStatusWithFallback(fileName, !enabled))
      );
      ...
      <MonitoringPanel className={styles.toolbarPanel}>
        ...
        <div className={styles.filterBar}>
          <div className={styles.filterGrid}>
            ...
            />
            <Select
              value={selectedStatus === 'all' ? 'all' : selectedStatus === 'success' ? 'success' : 'failed'}
        ...
        {!(config as Record<string, unknown>)?.usageStatisticsEnabled ? (
          <div className={styles.callout}>
          ...
          <table className={`${styles.table} ${styles.realtimeTable}`}>
            ...
            <tbody>
              {realtimePagination.pageItems.map((row) => {
                const sourceDisplay = buildRealtimeSourceDisplayLegacy(row, t);
                return (

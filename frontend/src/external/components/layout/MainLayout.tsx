...
import { MainRoutes } from '@/external/router/MainRoutes';
import { pluginsApi } from '@/services/api';
import { externalNavGroups } from '@/external/externalNav';
import {
...
interface SidebarNavGroup {
  ...
  subGroups?: SidebarNavGroup[];
}

const flattenNavItems = (items: SidebarNavItem[]): SidebarNavLinkItem[] =>
  items.flatMap((item) => (item.kind === 'drawer' ? item.children : [item]));

const flattenGroupItems = (group: SidebarNavGroup): SidebarNavLinkItem[] => {
  const own = flattenNavItems(group.items);
  const sub = (group.subGroups ?? []).flatMap((subGroup) => flattenGroupItems(subGroup));
  return [...own, ...sub];
};
...
export function MainLayout() {
  ...
  const [collapsedGroups, setCollapsedGroups] = useState<Set<string>>(() => new Set(['cluster']));
  ...
  const clusterGroup: SidebarNavGroup = {
    id: 'cluster',
  ...
  const navGroups: SidebarNavGroup[] = [
    ...
    ...externalNavGroups.map((group) => ({
      ...
      items: group.items.map((item) => ({
        ...item,
        icon: item.icon ?? sidebarIcons.monitoring,
      })),
    })),
    clusterGroup,
  ];
  const navItems = navGroups.flatMap((group) => flattenGroupItems(group));
        ...
        >
          ...
          <div className="nav-section">
            {navGroups.map((group, idx) => {
              const isSuperGroup = group.subGroups && group.subGroups.length > 0;
              ...
              if (isSuperGroup) {
                return (
                  <div
                    className="nav-group nav-super-group"
                    key={group.id}
                  >
                    <button
                      type="button"
                      className="nav-group-label nav-super-group-toggle"
                      onClick={toggleCollapse}
                      aria-expanded={!isCollapsed}
                    >
                      <span>{t(group.labelKey)}</span>
                      <span className={`nav-group-caret ${isCollapsed ? 'collapsed' : ''}`} aria-hidden="true">
                        <IconChevronDown size={14} />
                      </span>
                    </button>
                    {!isCollapsed && group.subGroups!.map((subGroup) => (
                      <div className="nav-sub-group" key={subGroup.id}>
                        {showSidebarLabels && (
                          <div className="nav-group-label">{t(subGroup.labelKey)}</div>
                        )}

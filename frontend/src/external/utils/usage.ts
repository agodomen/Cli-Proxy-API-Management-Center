...
export interface UsageDetailWithEndpoint extends UsageDetail {
  ...
  __timestampMs: number;
  /** 后端 event.provider — 当 channelByAuthIndex 匹配不到时作为 fallback */
  __backendProvider?: string;
  /** Original raw detail payload for debug tooltips */
  __rawItem?: Record<string, unknown>;
}
      ...
      modelDetails.forEach((detailRaw) => {
        ...
        details.push({
          ...
          statusCode: toPositiveNumber(
            detailRaw.status_code ?? detailRaw.statusCode ?? detailRaw.http_status ?? detailRaw.httpStatus
          ),
          errorMessage: readDetailString(

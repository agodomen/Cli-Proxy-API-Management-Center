...
} from '@/external/services/api/pluginProxy';
import { pluginStoreApi } from '@/external/services/api/pluginStore';
...
import { PluginInstallGateModal } from '@/features/plugins/components/PluginInstallGateModal';
import {
...
} from '@/features/plugins/pluginReleaseVersions';
import { waitForPluginStoreState } from '@/features/plugins/pluginPolling';
...
export function PluginStorePage() {
  ...
  const loadPluginProxy = useCallback(async () => {
    ...
    try {
      // Always resolve global proxy-url via the long-existing management endpoint,
      // so "system proxy" can display even when plugin-proxy API is unavailable.
      const systemUrl = await pluginProxyApi.getSystemProxyUrl();
      setSystemProxyUrl(systemUrl);

      try {
        const response = await pluginProxyApi.get();
  ...
  const proxyStatusOptions = useMemo(
    () => [
      {
        value: '0',
        label: t('plugin_store.proxy_mode_none'),
        title: t('plugin_store.proxy_mode_none_tip'),
      },
      {
        value: '2',
  ...
  const handleProxyStatusChange = useCallback(
    async (value: string) => {
      ...
      if (proxyUnsupported) {
        setProxyStatus(status);
        setProxyFeedback(null);
        // Keep system proxy url fresh for display.
        if (status === 2) {
          const systemUrl = await pluginProxyApi.getSystemProxyUrl();
          setSystemProxyUrl(systemUrl);
        }
      ...
      if (status === 1) {
        ...
        setProxyFeedback(null);
        if (!retained) {
          // Wait for user input + blur validation before enabling custom proxy.
          return;
        }
        try {
      ...
      try {
        // Refresh system proxy display when switching to system.
        if (status === 2) {
          const systemUrl = await pluginProxyApi.getSystemProxyUrl();
          setSystemProxyUrl(systemUrl);
        }
    ...
    [proxyStatus, proxySaving, proxyUnsupported, savePluginProxy, t]
  );

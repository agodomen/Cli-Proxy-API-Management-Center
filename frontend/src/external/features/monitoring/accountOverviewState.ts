...
export type MonitoringStatusFilter = 'all' | 'success' | 'failed' | number;  // number 是具体的 HTTP 状态码（如 0, 200, 429, 503 等）
...
const normalizeStatusFilter = (value: unknown): MonitoringStatusFilter =>
  value === 'success' || value === 'failed' ? value : typeof value === 'number' ? value : 'all';

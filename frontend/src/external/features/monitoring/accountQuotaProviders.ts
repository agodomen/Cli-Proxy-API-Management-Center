...
import type { GeminiCliQuotaBucketState } from '@/external/types/geminiCli';
import { requestCodexUsagePayload } from '@/external/services/api/codexQuota';
import {
  formatKimiResetHint,
...
/**
 * Maps an AntigravityQuotaGroup (new bucket-based structure) to the panel's
 * uniform AccountQuotaWindow shape. The first bucket's remainingFraction and
 * resetTime are used for the group-level display.
 */
const antigravityGroupToAccountWindows = (group: AntigravityQuotaGroup): AccountQuotaWindow[] => {
...
const fetchAntigravityEntry = async (
  ...
  const windows = groups.flatMap(antigravityGroupToAccountWindows);
  return {
    ...commonFields(target),
    subtitle: null,
    windows,

...
import type { ApiKeyAlias } from '@/external/services/api/usageService';
import type { AuthFileItem } from '@/types/authFile';
import type { Config } from '@/types/config';
import type { CredentialInfo } from '@/external/types/sourceInfo';
import { buildSourceInfoMap, resolveSourceDisplay } from '@/external/utils/sourceResolver';
...
export type MonitoringEventRow = {
  ...
  taskKey: string;
  searchText: string;
  /** Original usage-service page item / detail payload for debug tooltips */
...
const buildEventRows = (
  ...
  details
    .map((detail, index) => {
      ...
      const channelLabel =
        channelMeta?.name || authMeta?.provider || snapshotProvider || sourceMeta.type || readString(detail.__backendProvider) || (authIndex ? String(authIndex) : '-');
      ...
      return {
        ...
        apiKeyLabel,
        apiKeyMasked,
        provider: readString(detail.__backendProvider) || authMeta?.provider || snapshotProvider || sourceMeta.type || (authIndex ? String(authIndex) : '-'),
        ...
        rawItem: isRecord(detail.__rawItem) ? detail.__rawItem : undefined,
...
const buildRealtimeRowsFromPageItems = (
  ...
  const details = items.map((item) => {
    ...
    return {
      ...
      statusCode:
        readPageNumber(item, 'status_code', 'statusCode') ||
        readPageNumber(item, 'http_status', 'httpStatus'),
      errorMessage: readString(item.error_message || item.errorMessage || item.error),
      request_count: 1,
      ...
      __backendProvider: readString(item.provider) || undefined,
      __endpoint: endpoint,
      ...
      __rawItem: item,
    } as UsageDetailWithEndpoint & { __rawItem?: RecordLike };
  });
...
export function useMonitoringData({
  ...
  const filterFacets = useMemo<MonitoringFilterFacets>(() => {
    ...
    // When summary is skipped (realtime-only pages), derive facets from realtime page rows.
    const eventRowsForFacets =
      realtimePageRows && realtimePageRows.length > 0 ? realtimePageRows : filteredRows;

    const apiKeyOptions = new Map<string, string>();
    eventRowsForFacets.forEach((row) => {
    ...
    return {
      providers: Array.from(new Set(eventRowsForFacets.map((row) => row.provider))).filter(Boolean),
      accounts: buildAccountRows(eventRowsForFacets).map((row) => ({
        value: row.account,
        label:
      ...
      models: Array.from(new Set(eventRowsForFacets.map((row) => row.model))).filter(Boolean),
      channels: Array.from(new Set(eventRowsForFacets.map((row) => row.channel))).filter(Boolean),
      apiKeys: Array.from(apiKeyOptions.entries()).map(([value, label]) => ({ value, label })),
    };
  }, [filteredRows, realtimePageRows, summaryFilterFacets]);
